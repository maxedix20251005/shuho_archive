...
|---|---|---|---|---|---|---|
| `BLG-001` | `P1` | Finalise production domain and add canonical/`og:url` on all public pages | 本番ドメイン確定後、全公開ページへ canonical/`og:url` を追加 | `Open` | `Web Director` | `Before production release` |
| `BLG-002` | `P1` | Create and apply `supabase/config.js` with operation-safe key management | 運用上安全な鍵管理方針で `supabase/config.js` を作成・適用 | `Open` | `Tech Lead` | `Before staging integration` |
| `BLG-003` | `P2` | Connect admin pages to live Supabase CRUD (works/enquiries/news) | 管理画面（作品/問い合わせ/お知らせ）を Supabase 実CRUDへ接続 | `Open` | `Frontend Engineer` | `Sprint +1` |
